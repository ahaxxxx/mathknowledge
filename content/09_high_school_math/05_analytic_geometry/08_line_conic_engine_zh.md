...
先确认 $A\ne0$，方程才是二次方程；若 $A=0$，应回到一次方程或恒等式分别讨论，不能使用二次方程的韦达关系与判别式判交点数。

例如双曲线 $x^2-y^2=1$ 与 $y=x+1$ 联立，得到 $-2x-2=0$，只有一个交点 $(-1,0)$。此时形式上的 $B^2-4AC=4>0$ 并不代表两个交点。
...
2. 截距相关且两个截距均非零：设 $\frac xa+\frac yb=1$；过原点或平行于坐标轴时改用一般式。
3. 斜率可能不存在：用一般式或单独讨论。
4. 过焦点：根据焦点坐标设点斜式。
