...
:::solution 查看解析
两条直线的非零法向量分别为 $(2,a)$ 和 $(1,-1)$。同时把两条直线的方向旋转直角，夹角关系不变，所以两直线垂直等价于这两个法向量垂直。数量积为 $2-a=0$，得 $a=2$。这个方法不需要除以 $a$，也覆盖 $a=0$ 的情形。
:::
...
:::solution 查看解析
先确认消元等价、二次项系数非零，并检查根符合原变量限制。此时两个不同交点对应 $\Delta>0$，至少一个交点对应 $\Delta\ge0$。若二次项消失，应按一次方程等情形另算；例如 $x^2-y^2=1$ 与 $y=x+1$ 消元为 $-2x-2=0$，只有一个交点，不能套二次判别式。
:::
...
$$
二次项系数恒正，判别式为 $4k^2$。题设要求两个不同交点，因此 $k\ne0$；$k=0$ 时直线 $y=1$ 相切。
:::
...
:::solution 查看解析
由 $x^2\le4$，得 $-2\le x\le2$。反过来，任取该区间中的 $x$，令 $y=\sqrt{4-x^2}$，即得圆上一点。因此整个区间都能取到。
:::
...
:::solution 查看解析
由 $\frac{y^2}{9}\le1$，得 $-3\le y\le3$。反过来，任取该区间中的 $y$，令 $x=4\sqrt{1-y^2/9}$，即得椭圆上一点，故范围恰为 $[-3,3]$。
:::
...
:::solution 查看解析
圆心到直线距离为 $\frac2{\sqrt{k^2+1}}$。当 $|k|>\sqrt3$ 时距离小于 $1$，有两个交点；当 $|k|=\sqrt3$ 时相切；当 $|k|<\sqrt3$ 时相离。因此可以相交，条件是 $|k|>\sqrt3$。若问“有公共点”，条件才是 $|k|\ge\sqrt3$。
:::
