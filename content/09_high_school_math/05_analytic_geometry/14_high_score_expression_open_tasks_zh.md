...
应先合并同类项。若整理为关于 $k$ 的多项式恒等式，且允许的参数值有无限多个，才可令各幂次的系数同时为 0。一次式只需在至少两个不同参数值下成立。对任意函数 $A,B,C$，不能把原来的三项各自置零。

例如 $kx+(k+1)y-(2k+1)=0$ 应整理为 $k(x+y-2)+(y-1)=0$，再令 $x+y-2=0,y-1=0$，得到定点 $(1,1)$；原来的三项在该点并不分别为 0。
