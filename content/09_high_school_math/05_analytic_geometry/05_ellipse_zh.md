...
设两焦点为 $F_1(-c,0),F_2(c,0)$，其中 $a>c>0$。记 $r_1=PF_1,r_2=PF_2$。距离公式给出

$$
r_1^2-r_2^2=(x+c)^2-(x-c)^2=4cx.
$$

由 $r_1+r_2=2a$，且 $2a>0$，得到 $r_1-r_2=2cx/a$，所以

$$
r_1=a+\frac{cx}{a},\qquad r_2=a-\frac{cx}{a}.
$$

代入 $r_2^2=(x-c)^2+y^2$ 并展开消去一次项，得

$$
\frac{a^2-c^2}{a^2}x^2+y^2=a^2-c^2.
$$

令 $b^2=a^2-c^2>0$，除以 $b^2$ 即得标准方程。

这一步包含平方，因此还要反向证明：若点满足标准方程，则 $|x|\le a$，于是 $a\pm cx/a\ge a-c>0$。将方程代回可知两距离的平方分别等于 $(a\pm cx/a)^2$；因两边非负，距离正是 $a\pm cx/a$，和为 $2a$。所以标准方程与定义等价，没有增根。

焦半径公式也由这段推导得到；它依赖于焦点的左右标号，交换 $F_1,F_2$ 时两式也要交换。
