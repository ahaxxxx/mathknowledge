# 圆锥曲线大题十个大招

本页对应本地资料“终结圆锥曲线大题十个大招”。这里不把“大招”理解成套路口号，而是把圆锥曲线大题中反复出现的结构整理成可复用的方法。

## 1. 定义优先

遇到椭圆、双曲线、抛物线，先回到定义。焦点三角形、焦半径、准线距离，很多时候比联立更快。

## 2. 设直线要完整

常用 $y=kx+m$，但必须检查竖直线。过定点时用

$$
y-y_0=k(x-x_0)
$$

更稳。

## 3. 联立后先写韦达

先检查二次项系数非零，并记录判别式与变量限制；若降次则另行讨论。不要急着求根。高考大题通常不需要 $x_1,x_2$ 的具体值，而需要它们的和、积、差的平方。

## 4. 弦长公式模板

若直线斜率为 $k$：

$$
|PQ|=\sqrt{1+k^2}\sqrt{(x_1+x_2)^2-4x_1x_2}.
$$

若用 $x=t$，则直接用纵坐标差。

## 5. 面积转距离

三角形面积常用

$$
S=\frac12\cdot\text{底}\cdot\text{高}.
$$

底可以是弦长，高可以是点到直线距离。面积题不要一开始就硬代坐标。

## 6. 中点弦用点差法

两点在同一曲线上，代入后相减，可以得到弦斜率和中点坐标之间的关系。这比完整联立更轻。

## 7. 定点题看直线族

把直线方程整理成

$$
A(x,y)k+B(x,y)=0.
$$

若要对任意 $k$ 成立，就令 $A=0$，$B=0$。

## 8. 定值题看消参

把目标写成参数表达式，再用曲线关系、韦达关系消去参数。最后参数消失，才是真正的定值证明。

## 9. 最值题看约束

最值来自约束。约束可能是 $\Delta\ge0$、点在曲线上、参数有范围、距离非负或几何位置限制。

## 10. 特殊情况最后回补

斜率不存在、切线、重根、端点、焦点直线、坐标轴平行，这些是圆锥曲线大题常见漏分点。主线算完后必须回看。

## 大题讲题模板

1. 曲线方程和参数关系是什么？
2. 直线如何设，是否完整？
3. 联立得到哪个二次方程？
4. 韦达关系是什么？
5. 题目目标转成什么代数式？
6. 化简后是否出现定点、定值或范围？
7. 特殊情况是否补齐？

## 训练题

1. 为什么圆锥曲线大题常常不需要直接解出交点？

:::solution 查看解析
因为目标通常可以用两根和、两根积、两根差表示。韦达关系比显式解根更简洁。
:::

2. 面积题为什么常常要先找点到直线距离？

:::solution 查看解析
若一边是弦，另一点固定，则面积等于 $\frac12$ 弦长乘固定点到弦所在直线的距离，结构更清楚。
:::

3. 中点弦题为什么适合点差法？

:::solution 查看解析
点差法直接利用两点都在曲线上，把二次项相减后出现 $x_1+x_2$、$y_1+y_2$，而这些正好由中点给出。
:::

4. 定点题中，为什么可以令参数系数和常数项都为 $0$？

:::solution 查看解析
因为等式要对任意参数成立。只有参数项和非参数项同时为 $0$，才能保证所有参数都满足。
:::

5. 最值题中，为什么 $\Delta>0$ 和 $\Delta\ge0$ 要区分？

:::solution 查看解析
在联立确为二次方程且根均满足原始限制时，$\Delta>0$ 表示两个不同交点，$\Delta=0$ 表示相切或重根。题目若要求弦或两个交点，通常需要 $>0$；若只要求有公共点，可以用 $\ge0$。
:::

## 讲题追问

学生讲圆锥曲线大题时，老师不要先纠正计算，而是先追问：你设的参数是否覆盖所有情况？你真正要证明的不变量是什么？
