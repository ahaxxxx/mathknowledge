...
| 已知结构 | 优先方法 | 理由 |
| --- | --- | --- |
| 两角一边 | 正弦定理 | 角和能补第三角，有边角对应 |
| 两边夹角 | 余弦定理或面积公式 | 夹角触发 $\cos$ 或 $\sin$ |
| 三边 | 余弦定理 | 可直接求角或判断锐钝 |
| 两边和非夹角 | 正弦定理，但要查 SSA 多解 | 由正弦值反求角可能不唯一 |
| 出现面积 | 面积公式，再联动正余弦定理 | 面积需要两边夹角或等价信息 |
| 出现 $\sin A,\sin B$ 比例 | 边角互换 | 正弦定理可把正弦比例换成边比例 |
