...
若函数 $f$ 在区间上是凸的，且在内点 $a$ 可导，图像在该区间位于任意切线之上：
...
实际计算 $F(-1)=5$、$F(1)=1$，且两处均为正；又 $F$ 在 $(-\infty,-1)$ 递增、$(-1,1)$ 递减、$(1,+\infty)$ 递增，所以只可能在左侧有根。又 $F(-3)=-15<0$，$F(-1)=5>0$，由连续性保证左侧存在根，再由严格递增保证唯一，故 $F$ 恰有一个实根。这个例子提醒：罗尔定理给出的粗上界要结合极值符号才能收紧。
