...
设 $x_0$ 是定义域内点，$f$ 在 $x_0$ 连续，且在它左右某个去心邻域内可导。若 $f'$ 在 $x_0$ 左正右负，则 $x_0$ 为极大值点；左负右正，则为极小值点。这是充分条件。连续性把两侧的变化连接到 $f(x_0)$，不能省略：若 $f(x)=-|x|$（$x\ne0$），另设 $f(0)=-1$，两侧仍左正右负，但在足够小的邻域内 $f(0)$ 反而最小。
...
若 $f$ 在 $[a,b]$ 连续，则绝对最大、最小值一定存在；这里不要求每个内点都可导。候选点只可能来自：
