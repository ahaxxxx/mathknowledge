...
在相应最大值或最小值存在时，参数条件是 $a\ge\max_{x\in I}\varphi(x)$；若目标是 $a\le\varphi(x)$，则为 $a\le\min_{x\in I}\varphi(x)$。这一句的前提是移项、除法和乘法没有改变不等号方向，尤其要先确定除数的符号。若最值不取到，应使用上确界或下确界，或直接检查全称不等式。例如对所有 $x\in(0,1)$ 有 $a\ge x$，条件是 $a\ge1$，但 $x$ 在该区间没有最大值。
