...
若最小值存在，问题等价于 $\min_{x\in I}h(x)\ge0$。若不取到最小值，应直接证明每个函数值非负，或使用下确界（所有下界中最大的一个），不能默认存在最小值。例如 $h(x)=x$ 在 $(0,1)$ 上恒正，却没有最小值。若参数 $a$ 只出现在常数项，在相应最值存在时可整理为 $a\ge \max\varphi(x)$ 或 $a\le\min\varphi(x)$；方向由移项决定，不能凭感觉记。
