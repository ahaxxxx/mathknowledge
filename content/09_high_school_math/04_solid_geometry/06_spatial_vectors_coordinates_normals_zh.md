...
正方体、长方体可沿同一顶点处三条互相垂直的棱建系。一般直棱柱的底面棱未必垂直，不能直接把任意三条棱当作正交坐标轴；应在底面内另选两个互相垂直的方向，再以侧棱方向为第三轴。正棱锥常把底面中心作为原点，把底面放在 $xOy$ 平面，把高放在 $z$ 轴上。
...
一个非零向量 $\mathbf n$ 如果垂直于平面 $\alpha$ 内的两条相交直线，就叫平面 $\alpha$ 的法向量。
