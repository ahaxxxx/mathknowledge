...
上述实数指数形式先按实特征根讨论；无实根时需要复数或实三角形式，超出这里的基础范围。若出现零特征根，建议直接回到降阶递推处理，避免 $0^0$ 的约定问题。尤其重根为 0 时，递推变成 $a_{n+2}=0$，前两项由初值独立决定，不能套非零重根模板。例如初值为 $1,2$ 时，序列是 $1,2,0,0,\ldots$。
...
本方法只适用于可写成 $u a_na_{n+1}+v a_{n+1}+w a_n+z=0$ 的双线性递推，例如合法的分式线性递推交叉相乘后的形式；交叉相乘前后的分母限制必须保留。一般递推不能只因平移后常数项消失就化成这种结构，例如 $a_{n+1}=a_n^2$ 仍含纯平方项。

先把适用的递推式写成关于 $a_n,a_{n+1}$ 的方程
...
先得到 $P+QC_n+RC_{n+1}=0$。只有 $R\ne0$ 时，才能除以 $R$，得到一次线性递推

$$
C_{n+1}=-\frac QR C_n-\frac PR.
$$

若 $R=0$，只剩 $P+QC_n=0$，不能由此解出 $C_{n+1}$，须回到原递推讨论；若某个 $B_n=0$，同样不得取倒数。
...
如果题目明确要求每一项 $a_n>0$，就能排除负值分支。仅初值为正并不能保证以后每项为正，必须结合递推条件逐步判断。
...
12. $a_n=2^{n-1}$。
13. $a_n=n2^{n-1}$。
14. $a_n=\frac{4}{n(n+1)}$。由累乘得 $a_n=2\prod_{k=1}^{n-1}\frac{k}{k+2}=\frac{4}{n(n+1)}$，代入 $n=1$ 也满足初值。
