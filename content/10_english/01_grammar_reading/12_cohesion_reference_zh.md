...
- 同一对象常被重新命名，如 `the intervention`、`the program`、`this policy`。
...
`The sample was small. This reduced precision.` 中 `This` 指什么？
...
:::solution 查看解析
`did not` 替代 `improve`；`whereas` 明示两组对比。它没有自动说明差异由 treatment 导致，因果仍需设计支持。
:::
...
:::solution 查看解析
`it` 可指 model，也可指 benchmark；两者语法数和语义都匹配。应明确写 `after the model was revised` 或 `after the benchmark was revised`。
:::
...
解释 `the former`：`We distinguish prediction from explanation. The former can succeed without the latter.`
...
:::solution 查看解析
`her` 与 `she` 都可能在语法上指 Maya 或 Ana，常识不能保证唯一答案。应重写明确所有者和迟到者。
:::

### 训练 8
**训练语境：真实生活**

`I missed the bus. That ruined my morning.` 中 `That` 指什么？
