...
中心词是单数 `expansion`，所以谓语用 `has`；`programs` 虽离谓语近，却只是 `of` 短语内部名词。
...
`evidence` 通常不可数，谓语用单数 `supports`；`sources` 不控制主谓一致。
...
:::solution 查看解析
中心词是复数 `effects`，因此谓语用 `remain`。`exposure` 与 `noise` 都嵌在介词短语中，不控制全句一致。
:::

### 训练 2
**训练语境：学术科研**

比较 `few experiments` 与 `a few experiments`。
...
:::solution 查看解析
可能每位学生各读一篇，不要求同一篇；也可能存在一篇所有人都读的论文。`every` 与 `a` 的量化范围需要语境或改写明确。
:::
...
比较 `I need a key` 与 `I need the key`。
...
:::solution 查看解析
它可能被理解为“两扇门都没锁”，也可能被理解为“并非两扇都锁了”。为避免否定范围歧义，应写 `Neither door is locked` 或 `Not both doors are locked`。
:::
